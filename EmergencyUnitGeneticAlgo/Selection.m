...
classdef Selection
    % Selection class
    % Determines the coordinates in the next generation population
    ...
    methods(Static)
        function bestFitness = tournamentSelection(population)
            ...
            bestFitness = population.getCoordinate(randi(population.getSize()));
            
            % Loop through tournament size (k)
            for i = 1 : TOURNAMENT_SIZE
                % Get a random word in the population
                contFitness = population.getCoordinate(...
